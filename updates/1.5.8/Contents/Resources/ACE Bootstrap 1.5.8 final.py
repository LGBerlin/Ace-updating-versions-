...
1.5.6 Stop/Preview services, then fixes poster Preview Edit so the toolbar Edit
button opens the real PPTX-backed poster editor instead of the legacy slide-only alert.
"""
...
JS158 = r'''
(function(){
  if(window.__ACE158_EDIT_FIX_FINAL__)return;window.__ACE158_EDIT_FIX_FINAL__=1;
  ...
  function bind158(){
    ...
    [...root.querySelectorAll('button,a,[role="button"]')].filter(isEdit158).forEach(old=>{
      if(old.dataset.ace158final==='1')return;
      const b=old.cloneNode(true);b.dataset.ace158final='1';
...
patch(H / 'index.html', 'ACE158_FINAL', idx158)
patch(H / 'app.css', 'ACE158_FINAL', lambda s: s + '\n/*ACE158_FINAL*/\n')
patch(H / 'app.js', 'ACE158_FINAL', lambda s: s + '\n' + JS158 + '\n// ACE158_FINAL\n')
