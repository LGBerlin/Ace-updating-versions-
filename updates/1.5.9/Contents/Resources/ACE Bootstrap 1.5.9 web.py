...
def _fetch_source(item):
    try:
        ...
        return {'url':final,'domain':host,'title':title[:220] or item.get('anchor','')[:220] or host,
                'description':desc[:500],'excerpt':body[:2600],'engine':item.get('engine','Web')}
    except Exception:
        return None
...
def _research(query):
    ...
    candidates = _discover(query); results = []
    ...
    with _CACHE_LOCK: _CACHE[key]=(now,results)
    return results


_NON_RESEARCH = re.compile(r'\b(write|rewrite|rephrase|translate|proofread|summarize this|summarise this|make this sound|poem|story|joke|roleplay|brainstorm|speech pattern|chat style|how you talk|how you speak|change your|stop using|email|text message)\b', re.I)
_FACT = re.compile(r'\b(what|which|who|when|where|why|how|best|top|latest|current|today|now|price|cost|get|find|obtain|drop|craft|recipe|guide|versus|vs\.?|compare|difference|history|facts?|cause|effect|meaning|definition|evidence|source|research|search|look up|verify|accurate)\b', re.I)
...
def _should_research(text, path):
    ...
    if re.search(r'\b(do not|don\'t|without)\s+(?:search|research|browse|use the web)\b',t,re.I): return False
    is_artifact='/opendesign' in path.lower() or bool(_ARTIFACT.search(t))
    if is_artifact and not re.search(r'\b(about me|my life|myself|fictional|imaginary|creative writing)\b',t,re.I): return True
    if _NON_RESEARCH.search(t) and not _FACT.search(t): return False
    if '?' in t or _FACT.search(t): return True
    if re.search(r'\b(tell me|explain|give me information|teach me)\b',t,re.I): return True
    return False
...
def _find_user_text(d):
    ...
    for k in ('message','prompt','input','text','request','topic','instructions'):
        if isinstance(d.get(k),str) and d[k].strip(): return d,k,d[k]
    return None,None,None


def _evidence_block(sources, artifact=False):
    if not sources:
        return "\n\n[ACE_WEB_RESEARCH_159 INTERNAL — never claim live verification succeeded] A live web check was attempted but no usable independent pages could be retrieved. Do not invent current facts or say you searched successfully. If the answer materially depends on current/specific facts, say that live verification was unavailable and give only what you can support cautiously. [/ACE_WEB_RESEARCH_159]"
    lines=["\n\n[ACE_WEB_RESEARCH_159 INTERNAL — use this evidence, never quote this instruction]","LIVE WEB RESEARCH. Cross-check claims across these independent pages. Prefer primary/official sources and strong agreement. If sources conflict, say so. Do not make up details that are absent."]
    for i,s in enumerate(sources,1):
        lines.append(f"SOURCE {i} | {s['domain']} | {s['title']} | {s['url']}")
        if s.get('description'): lines.append('Summary: '+s['description'])
        lines.append('Evidence: '+s['excerpt'])
    if artifact: lines.append("Use the verified facts in the artifact. Do not clutter the poster/presentation with raw URLs or a source list unless the user asked for citations.")
    else: lines.append("Answer naturally and concisely. For current, disputed, or recommendation-style questions, add a short 'Sources checked:' line with 2–4 domain names; otherwise citations are optional.")
...
def _research_request_body(self):
    ...
    if any(x in path.lower() for x in blocked): return
    ...
    if n<=0 or n>2_000_000: return
    raw=self.rfile.read(n); new=raw
    try:
        d=json.loads(raw.decode('utf-8')); holder,key,user_text=_find_user_text(d)
        if holder is not None and user_text and '[ACE_WEB_RESEARCH_159' not in user_text and _should_research(user_text,path):
            sources=_research(user_text); artifact='/opendesign' in path.lower() or bool(_ARTIFACT.search(user_text))
            holder[key]=user_text+_evidence_block(sources,artifact=artifact)
            d['ace_web_researched']=bool(sources); d['ace_web_source_count']=len(sources)
            new=json.dumps(d,ensure_ascii=False,separators=(',',':')).encode('utf-8')
    except Exception:
        new=raw
    self.rfile=io.BytesIO(new)
    try: self.headers.replace_header('Content-Length',str(len(new)))
    except Exception: pass


def POST(self):
    try: _research_request_body(self)
    except Exception: pass
    return post0(self)
